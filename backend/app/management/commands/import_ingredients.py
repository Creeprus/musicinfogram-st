...
class Command(BaseCommand):
    """Класс, в котором описана команда импортирования ингредиентов
    для manage.py"""
    ...
    def import_from_csv(self, file_path):
        """Функция, которая импортирует ингредиенты из CSV файла."""
        try:
    ...
    def import_from_json(self, file_path):
        """Функция, которая импортирует ингредиенты из JSON файла."""
        try:
            with open(file_path, 'r', encoding='utf-8') as file:
                data = json.load(file)
                ingredients = []
